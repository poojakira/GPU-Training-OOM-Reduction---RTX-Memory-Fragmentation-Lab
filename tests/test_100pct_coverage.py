...
Surgically targets every remaining uncovered line across the gpudefrag codebase
...
class TestCLICoverage:
    ...
    def test_cli_no_rich_import_error(self):
        ...
        with patch.dict("sys.modules", {"rich": None, "rich.console": None, "rich.panel": None, "rich.text": None}):
            if "gpudefrag.cli" in sys.modules:
                ...
                from gpudefrag.cli import _print, print_banner, HAS_RICH
                with patch("gpudefrag.cli.HAS_RICH", False), \
                     patch("gpudefrag.cli.console", None), \
    ...
    def test_cli_collect_error_branch(self):
        ...
        from gpudefrag.cli import collect_cmd
        with patch("sys.argv", ["gpudefrag-collect", "--model", "gpt2", "--iterations", "1"]), \
             patch("gpudefrag.profiler.collector.collect_from_model", side_effect=RuntimeError("Collection Fail")), \
...
class TestDashboardCoverage:
    ...
    def test_dashboard_module_main_block(self):
        ...
        from gpudefrag import dashboard
...
class TestBenchmarkTritonCoverage:
    ...
    def test_run_benchmark_no_triton(self):
        ...
        from gpudefrag.defrag_engine import benchmark_triton
...
class TestDefragmenterCoverage:
    ...
    def test_defrag_triton_import_path(self):
        ...
        from gpudefrag.defrag_engine.defragmenter import HAS_TRITON
        if not HAS_TRITON:
            # The fallback function should raise RuntimeError
            from gpudefrag.defrag_engine.defragmenter import triton_compaction_copy
...
class TestKernelsCoverage:
    ...
    def test_analyze_fragmentation_empty(self):
        ...
        from gpudefrag.defrag_engine.kernels import analyze_fragmentation_triton
...
class TestPolicyCoverage:
    ...
    def test_mitigation_action_to_dict(self):
        ...
        from gpudefrag.defrag_engine.policy import MitigationAction
...
class TestQuantizationCoverage:
    ...
    def test_quantization_with_cuda_available(self):
        ...
        from gpudefrag.optimization.quantization import apply_gpu_quantization
    ...
    def test_get_model_size_mb_with_buffers(self):
        ...
        from gpudefrag.optimization.quantization import get_model_size_mb
...
class TestAllocatorLoggerCoverage:
    ...
    def test_mem_stats_cuda_available(self):
        ...
        from gpudefrag.profiler.allocator_logger import _mem_stats
        with patch("gpudefrag.profiler.allocator_logger._cuda_available", return_value=True), \
...
class TestCollectorCoverage:
    ...
    def test_collect_from_model_resnet50(self):
        ...
        with patch("gpudefrag.profiler.collector.ensure_cuda"), \
             patch("gpudefrag.trainer._models.build_resnet50", return_value=(mock_model, mock_inputs)), \
             patch("torch.cuda.is_available", return_value=False), \
    ...
    def test_collect_from_model_bert(self):
        ...
        with patch("gpudefrag.profiler.collector.ensure_cuda"), \
             patch("gpudefrag.trainer._models.build_bert", return_value=(mock_model, mock_inputs)), \
             patch("torch.cuda.is_available", return_value=False), \
             patch("torch.cuda.memory_allocated", return_value=0), \
             patch("torch.cuda.memory_reserved", return_value=0), \
             patch("torch.cuda.synchronize"):
    ...
    def test_collect_from_model_iteration_logging(self):
        ...
        with patch("gpudefrag.profiler.collector.ensure_cuda"), \
             patch("gpudefrag.trainer._models.build_gpt2", return_value=(mock_model, mock_inputs)), \
             patch("torch.cuda.is_available", return_value=False), \
             patch("torch.cuda.memory_allocated", return_value=0), \
             patch("torch.cuda.memory_reserved", return_value=0), \
             patch("torch.cuda.synchronize"):
...
class TestMonitorCoverage:
    ...
    def test_monitor_load_model_exception_fallback(self):
        ...
        with patch("os.path.exists", return_value=False), \
             patch.dict("sys.modules", {"gpudefrag.scheduler.default_weights": MagicMock(DEFAULT_WEIGHTS_B64="invalid==")}):
    ...
    def test_monitor_predict_snapshot_parsing(self):
        ...
        with patch("gpudefrag.utils.parse_memory_snapshot", return_value={"frag_score": 0.9}), \
             patch("torch.cuda.is_available", return_value=False):
...
class TestAutoInstrumentCoverage:
    ...
    def test_auto_instrument_heartbeat_exception(self):
        ...
        with patch("torch.cuda.is_available", return_value=True), \
             ...
             patch("gpudefrag.trainer.auto_instrument.DDPSyncManager"):
...
class TestUtilsCoverage:
    ...
    def test_get_cuda_info_exception(self):
        ...
        from gpudefrag.utils import get_cuda_info
    ...
    def test_parse_memory_snapshot_no_cuda(self):
        ...
        from gpudefrag.utils import parse_memory_snapshot
        with patch("torch.cuda.is_available", return_value=False):
