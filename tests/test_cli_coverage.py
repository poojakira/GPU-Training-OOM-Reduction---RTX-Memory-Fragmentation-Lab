...
from gpudefrag.cli import main, collect_cmd, train_cmd, benchmark_cmd, serve_cmd, dashboard_cmd

# 1. Main Subcommand Branches
def test_cli_mock_telemetry_branch():
    """Verify mock-telemetry subcommand branch."""
    with patch("sys.argv", ["gpudefrag", "mock-telemetry", "--interval", "0.01"]), \
         patch("gpudefrag.defrag_engine.defragmenter.GPUMemoryDefragmenter") as mock_engine_cls, \
...
def test_cli_status_branch():
    ...
    with patch("sys.argv", ["gpudefrag", "status"]), \
         ...
         patch("gpudefrag.utils.DefragConfig.load"), \
...
def test_cli_dashboard_branch_main():
    ...
    with patch("sys.argv", ["gpudefrag", "dashboard"]), \
         patch("gpudefrag.dashboard.DashboardManager") as mock_mgr_cls:
...
def test_train_cmd():
    ...
    with patch("sys.argv", ["gpudefrag-train", "--epochs", "1"]), \
...
def test_benchmark_cmd():
    ...
    with patch("sys.argv", ["gpudefrag-benchmark", "--runs", "1"]), \
...
def test_serve_cmd():
    ...
    with patch("sys.argv", ["gpudefrag-serve"]), \
...
def test_cli_error_handling():
    ...
    with patch("sys.argv", ["gpudefrag", "unknown"]), \
