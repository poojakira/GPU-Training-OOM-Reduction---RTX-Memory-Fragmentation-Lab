...
def test_api_telemetry_retry_failure():
    ...
    with patch("gpudefrag.api.Path.exists", return_value=True), \
...
def test_api_risk_scoring_tiers():
    ...
    with patch("gpudefrag.api._risk_model.score") as mock_score:
        # tier SAFE
...
def test_gpu_snapshot_success():
    ...
    from gpudefrag.api import _gpu_snapshot
...
def test_api_risk_history_cleared():
    ...
    from gpudefrag.api import _risk_model
...
def test_api_benchmarks_no_dir():
    ...
    client = TestClient(app)
    with patch("gpudefrag.api.Path.exists", return_value=False):
        response = client.get("/api/benchmarks")
        assert response.status_code == 200
...
def test_api_catch_all_file(tmp_path):
    ...
    with patch("gpudefrag.api.DASHBOARD_PATH", dist):
         client = TestClient(app)
