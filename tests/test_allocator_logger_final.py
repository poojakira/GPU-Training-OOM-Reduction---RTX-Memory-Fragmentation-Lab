...
from gpudefrag.profiler.allocator_logger import AllocatorLogger, _cuda_available, _mem_stats

def test_allocator_logger_cuda_unavailable():
    """Verify memory stats when CUDA is explicitly unavailable (Line 41-42)."""
    with patch("gpudefrag.profiler.allocator_logger._cuda_available", return_value=False):
        stats = _mem_stats()
